    ...
    <div class="row">
        <div class="col-lg-6">
            <div class="alert alert-info">
                <i class="fa fa-info-circle"></i> Here are the list of Attribute(s) in this particular Attribute
                Category.<br/>
                <a href="{{route('admin.eav.attribute.create')}}"><i class="fa fa-plus"> </i> Add New Attribute to this
                    category</a>
                <a class="pull-right" href="{{route('admin.eav.category.index')}}"><i class="fa fa-retweet"> </i> Back
                    to All Attribute category</a>
            </div><!--alert info-->

            <div class="dd permission-hierarchy">
                <ol class="dd-list">
                    <li class="dd-item" data-id="{!! $category->id !!}">
                        <div class="dd-handle">{!! $category->attribute_set_name !!} <span class="pull-right">{!! $category->product_attributes->count() !!}
                                Attributes</span></div>
                        @if ($category->product_attributes->count())
                            <ol class="dd-list">
                                @foreach($category->product_attributes as $cat)
                                    <li class="dd-item" data-id="{!! $cat->id !!}">
                                        <div class="dd-handle">{!! $cat->title !!} { {!! $cat->datatype!!} } <span
                                                    class="pull-right">{!! $cat->notnull ? "NotNull" : "NULL" !!} </span>
                                        </div>
                                    </li>
                                @endforeach
                            </ol>
